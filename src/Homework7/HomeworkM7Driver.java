...
public class HomeworkM7Driver {
	...
	public static boolean qNotFollowedByU(String word) {
		// YOUR CODE HERE
		if(recursiveCount(word, 'q','u',0)<1)
			return false;
		else
			return true;
	}
	...
	private static void recursiveMethod(int [] a, int first, int second){
		if(first<second){
			...
			a[first]=a[second];
			a[second]=temp;
			recursiveMethod(a,++first,--second);
	...
	public static int countPositives(BagInterface<Integer> bag) {
		return recursiveCount(bag,0);
		// YOUR EXTRA CREDIT CODE HERE
	}
	private static int recursiveCount(BagInterface<Integer> bag, int count){
		int positive =0;
		int b = bag.getCurrentSize();
		if (!bag.isEmpty()){
			int a = bag.remove();
			if(a>0){
				positive=1;
			}
			positive=positive+recursiveCount(bag, count+1);
			bag.add(a);
		}
		return positive;
	}
